...
public class Indexador {
    ...
    public void set(long chave, long endereco) {
        ...
        if (indice_status == Armazenador.MARCADOR_INDICE) {

            if (chave < Armazenador.MAX_ITENS_DO_INDICE) {
                long pos = chave * 8L;
                mArquivador.setPonteiro(mLocalIndice + (1L + 16L) + pos);
                mArquivador.set_u64(endereco);
                foi_lido = true;
            } else {
                long pos = Armazenador.MAX_ITENS_DO_INDICE * 8L;
                mArquivador.setPonteiro(mLocalIndice + (1L + 16L) + pos);
                escritor += Armazenador.MAX_ITENS_DO_INDICE;
            }

            limite_de_chaves += Armazenador.MAX_ITENS_DO_INDICE;
            chaves_acumuladas += Armazenador.MAX_ITENS_DO_INDICE;
    ...
    public long getIndiceItensTotalContagem() {
        return getIndicePaginasContagem() * (long) Armazenador.MAX_ITENS_DO_INDICE;
    }
