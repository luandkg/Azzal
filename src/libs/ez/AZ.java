...
public class AZ {
    ...
    public AZ(String arquivo_banco) {
    ...
    public void colecoes_criar(String colecao_nome) {
        ...
        Banco s_inits = AQSequenciador.organizar_banco(mArmazenador, "@Init");
        Banco s_bancos = AQSequenciador.organizar_banco(mArmazenador, "@Bancos");
        Banco s_sequencias = AQSequenciador.organizar_banco(mArmazenador, "@Sequencias");
        Banco s_auto_analises = AQSequenciador.organizar_banco(mArmazenador, "@AutoAnalise");
        Banco s_analises = AQSequenciador.organizar_banco(mArmazenador, "@Analise");
        Banco s_views = AQSequenciador.organizar_banco(mArmazenador, "@Views");
        ...
        for (ItemDoBanco item : s_bancos.getItens()) {
            ...
            if (obj_colecao.identifique("Status").isValor("DESTRUIDO")) {
                ...
                AQSequenciador.zerar_sequencial(s_sequencias, nome_antigo);
    ...
    public Banco colecoes_remover(String colecao_nome) {
        ...
        for (ItemDoBanco item : s_bancos.getItens()) {
            ...
            if (obj_colecao.identifique("Status").isValor("OK") && nome.contentEquals(colecao_nome)) {
                ...
                AQSequenciador.zerar_sequencial(s_sequencias, colecao_nome);
    ...
    public AQColecao colecoes_obter(String colecao_nome) {
        ...
        for (Banco b : colecoes_listar()) {
            if (b.getNome().contentEquals(colecao_nome)) {
                return new AQColecao(colecao_nome, mArmazenador, s_sequencias, b);
            }
        }
    ...
    public Lista<Banco> getColecoesInternas(){
    ...
    public void colecoes_organizar(String colecao){

        if (!colecoes_existe(colecao)) {
            colecoes_criar(colecao);
        }

    }

    public Entidade nova_entidade(){
        return new Entidade();
    }
    ...
    public void zerar(){
        ...
        for(Banco banco : colecoes_listar()){
            banco.limpar();
        }

        for(Banco banco : getColecoesInternas()){
            banco.limpar();
        }
